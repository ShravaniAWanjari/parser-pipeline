...
import openpyxl
import time
from sheet_insights.dashboard import dashboard_data
...
@app.get('/download/structured_dashboard')
def download_structured_dashboard():
    """Download structured JSON generated by GPT-4.1"""
    structured_path = Path("results/structured_dashboard.json")
    if not structured_path.exists():
        raise HTTPException(status_code=404, detail='Structured dashboard file not found')
    return FileResponse(path=structured_path, filename="structured_dashboard.json", media_type='application/json')
